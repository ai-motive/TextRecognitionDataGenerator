...
import string
import sys
from common.general_utils import get_ini_parameters
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        "--console_logging_", default=False, action='store_true', help="Activate logging"
    )
...
def update_args_by_ini(args, ini):
    args.language = ini['language']
    args.count = int(ini['count'])
    args.random_sequences = True if ini['random_sequences'] == 'True' else False
    args.include_letters = ini['include_letters'] if ini['include_letters'] else False
    args.include_numbers = ini['include_numbers'] if ini['include_numbers'] else False
    args.include_symbols = ini['include_symbols'] if ini['include_symbols'] else False
    args.length = int(ini['length'])
    args.random = ini['random'] if ini['random'] else False
    args.format = int(ini['format'])
    args.thread_count = int(ini['thread_count'])
    args.extention = ini['extention']
    args.skew_angle = int(ini['skew_angle'])
    args.random_skew = ini['random_skew'] if ini['random_skew'] else False
    args.use_wikipedia = ini['use_wikipedia'] if ini['use_wikipedia'] == 'True' else False
    args.blur = int(ini['blur'])
    args.random_blur = ini['random_blur'] if ini['random_blur'] else False
    args.background = int(ini['background'])
    args.handwritten = ini['handwritten'] if ini['handwritten'] else False
    args.name_format = int(ini['name_format'])
    args.output_mask = int(ini['output_mask'])
    args.distorsion = int(ini['distorsion'])
    args.distorsion_orientation = int(ini['distorsion_orientation'])
    args.width = int(ini['width'])
    args.alignment = int(ini['alignment'])
    args.orientation = int(ini['orientation'])
    args.text_color = ini['text_color'] if ini['text_color'] else '#282828'
    args.space_width = float(ini['space_width'])
    args.character_spacing = int(ini['character_spacing'])
    args.margins = margins(ini['margins']) if ini['margins'] else (5, 5, 5, 5)
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        if SELF_TEST_:
            ...
            sys.argv.extend(["--output_dir", OUT_PATH])
            sys.argv.extend(["--input_file", IN_PATH])
